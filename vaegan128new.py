...
class VAEGAN_MODEL128new(object):
    ...
    def train(self, train_loader):
        self.t_begin = t.time()
        # if not os.path.exists('vaegan128newh2z/'):
        #     os.makedirs('vaegan128newh2z/')
        generator_iter = 159600
        ...
        for epoch in range(self.epochs):
            ...
            for i, (images,_) in enumerate(train_loader):
                ...
                # loss4 = self.BCEloss(score_randn_gen.flatten() , real_labels)
                # loss5 = self.BCEloss(score_latentz_gen.flatten() , real_labels)
                # decoder_loss = self.gamma * recon_loss - discriminator_loss
                decoder_loss = recon_loss - discriminator_loss
                self.D.zero_grad()
                ...
                if generator_iter % 300 == 0:
                    ...
                    # sample_list = []
                    # for i in range():
                    #     z = Variable(torch.randn(5, latent_dim)).cuda(self.cuda_index)
                    #     samples = self.D(z)
                    ...
                    utils.save_image(grid, 'vaegan128new/randn_z_iter_{}.png'.format(str(generator_iter).zfill(3)))
                    # Denormalize images and save them in grid 8x8
                    ...
                    utils.save_image(grid, 'vaegan128new/sampled_z_iter_{}.png'.format(str(generator_iter).zfill(3)))
                    time = t.time() - self.t_begin
                    #print("Inception score: {}".format(inception_score))
                    print("Generator iter: {}".format(generator_iter))
                    print("Time {}".format(time))
                    dict = {'iters':[generator_iter],'encloss':[encoder_loss.cpu().data.numpy()],'decloss':[decoder_loss.cpu().data.numpy()],'disloss':[discriminator_loss.cpu().data.numpy()]}
                    df=pd.DataFrame(dict)
                    df.to_csv('gallery/vaegan128new_loss_inscore_h2z.csv', mode='a', index=False, header=False)
    ...
    def load_model(self):
        encoder_path = os.path.join('VAEGAN128new celebA128 30 32','encoder.pkl')
        decoder_path = os.path.join('VAEGAN128new celebA128 30 32', 'decoder.pkl')
        discriminator_path=os.path.join('VAEGAN128new celebA128 30 32', 'discriminator.pkl')
